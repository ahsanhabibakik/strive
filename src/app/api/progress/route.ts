...
export async function POST(request: NextRequest) {
  try {
    ...
    const { goalId, title, currentValue, notes, difficulty, confidence, motivation, mood, timeSpent, attachments, tags, nextSteps, challenges, lessons } = body;
    ...
    const latestProgress = await Progress.findOne({ goalId, userId: user._id })
      .sort({ dateRecorded: -1 });
