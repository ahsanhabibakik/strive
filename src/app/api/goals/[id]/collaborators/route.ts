...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const isCollaborator = goal.collaborators.some(
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({
      ...
      collaborator: updatedGoal.collaborators.find(
        c => c.userId._id.toString() === collaboratorUser._id.toString()
      ),
    });
  } catch (error) {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (goal.userId.toString() !== user._id.toString()) {
      return NextResponse.json(
        { error: "Only goal owner can update collaborator roles" },
        { status: 403 }
      );
    }
    ...
    const collaboratorIndex = goal.collaborators.findIndex(
