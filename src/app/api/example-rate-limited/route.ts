 ...
 */
export const GET = basicRateLimit(asyncHandler(async (request: NextRequest) => {
  // Your API logic here
 ...
 */
export const POST = requireAuth(userRateLimit(asyncHandler(async (request: NextRequest & { user?: any }) => {
...
const customRateLimit = createRateLimit({
  ...
  onLimitReached: (req: NextRequest) => {
    ...
    console.log('Rate limit reached for custom endpoint:', {
 ...
 */
export const PUT = customRateLimit(asyncHandler(async (request: NextRequest) => {
  const body = await request.json();
