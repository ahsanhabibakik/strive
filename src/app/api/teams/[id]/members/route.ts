...
export async function POST(
  ...
  try {
    ...
    const canInvite = team.settings.allowMemberInvites && team.members.some(m => 
    ...
    const isExistingMember = team.members.some(m => 
      m.userId.toString() === memberUser._id.toString()
    );
    ...
    const newMember = updatedTeam.members.find(m => 
      m.userId._id.toString() === memberUser._id.toString()
    );
...
export async function PUT(
  ...
  try {
    ...
    if (!isOwner && !isAdmin) {
      return NextResponse.json({ error: 'Permission denied to update member roles' }, { status: 403 });
    }
