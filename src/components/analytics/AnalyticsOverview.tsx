            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-blue-600">
                  ${overview.totalUsers > 0 ? (overview.monthlyRevenue / overview.totalUsers).toFixed(2) : '0'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Growth Rate</span>
                <span className="text-lg font-semibold text-purple-600">
                  {overview.totalUsers > 0 ? ((overview.newUsersThisMonth / overview.totalUsers) * 100).toFixed(1) : '0'}%
                </span>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="mt-4">
            <div className="flex h-2 rounded-full overflow-hidden">
              {planDistribution.map((plan) => {
                const width = overview.totalUsers > 0 
                  ? (plan.count / overview.totalUsers) * 100
                  : 0;
