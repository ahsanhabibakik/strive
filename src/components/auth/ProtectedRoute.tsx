          ...
          <div className="space-y-3">
            {!session ? (
              <Button asChild className="w-full">
                <Link href={`/auth/signin?callbackUrl=${encodeURIComponent(window.location.pathname)}`}>
                  Sign In
                </Link>
              </Button>
            ) : allowedRoles.length > 0 ? (
...
export const ModeratorRoute: React.FC<{ children: React.ReactNode; fallback?: React.ReactNode }> = ({ 
  children, 
  fallback 
}) => (
