...
export function RecentActivity({ user: _user }: RecentActivityProps) {
  ...
  return (
    <div className="bg-white shadow-2xs rounded-lg">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Recent Activity</h3>
        <p className="mt-1 text-sm text-gray-500">Latest events and actions in your application</p>
      </div>
      ...
      <div className="p-6">
                ...
                <li key={activity.id}>
                  <div className="relative pb-8">
                    ...
                    <div className="relative flex items-start space-x-3">
                      <div
                        className={`relative px-1 ${severityColors[activity.severity || "info"]} rounded-lg`}
                      >
                      ...
                      <div className="min-w-0 flex-1">
                        <div>
                          ...
                          {activity.user && (
                            <p className="mt-0.5 text-xs text-gray-400">by {activity.user.name}</p>
                          )}
                        </div>
        ...
        <div className="mt-6">
          <button
            type="button"
            className="w-full rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
          >
            View all activity
          </button>
        </div>
