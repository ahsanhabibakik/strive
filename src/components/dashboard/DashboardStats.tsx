...
export function DashboardStats({ stats, user }: DashboardStatsProps) {
  ...
  const visibleStats = statsConfig.filter(stat => 
    !stat.permission || RBAC.hasPermission(user, stat.permission)
  );
      ...
      {visibleStats.map((stat) => {
        ...
        return (
          ...
          >
            ...
            {/* Hover effect */}
            <div className="absolute inset-0 bg-gradient-to-r from-transparent to-gray-50 opacity-0 hover:opacity-100 transition-opacity" />
          </div>
        );
      })}
    </div>
