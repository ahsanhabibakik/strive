...
function checkEnvironment() {
  ...
  if (missingVars.length > 0) {
    ...
    log('Please set these variables in your .env.local file or deployment environment.', colors.yellow);
    process.exit(1);
  }
...
function validateDependencies() {
  ...
  try {
    ...
    const lockFile = fs.existsSync('pnpm-lock.yaml') ? 'pnpm-lock.yaml' : 
                    fs.existsSync('yarn.lock') ? 'yarn.lock' : 'package-lock.json';
    ...
    try {
      ...
      log('⚠️  Security vulnerabilities detected. Consider running "pnpm audit fix"', colors.yellow);
    }
...
function runTests() {
  ...
  testFiles.forEach(pattern => {
    try {
      const result = execSync(`find . -path "./node_modules" -prune -o -name "${pattern.replace('**/*', '*')}" -type f -print`, { encoding: 'utf8' });
      if (result.trim()) {
        hasTests = true;
      }
    } catch (error) {
  ...
  if (hasTests) {
    ...
    if (fs.existsSync('coverage/coverage-summary.json')) {
      ...
      if (total.lines.pct < minCoverage) {
        log(`⚠️  Code coverage (${total.lines.pct}%) is below threshold (${minCoverage}%)`, colors.yellow);
      }
    }
  } else {
